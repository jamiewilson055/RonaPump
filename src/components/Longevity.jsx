...
export default function Longevity({ session, onAuthRequired }) {
  ...
  return (
    <div className="lon-section">
      ...
      <div className="lon-vital" onClick={() => testedCount > 0 && setShowAnalytics(true)} style={{ cursor: testedCount > 0 ? 'pointer' : 'default' }}>
        ...
        <div className="lon-vital-info">
          ...
          {testedCount > 0 ? (
            <>
              ...
              <div className="lon-vital-sub">{testedCount}/{MARKERS.length} tested • Tap for analysis</div>
            </>
          ) : (
            <div className="lon-vital-sub">Complete your first test to see your Vital Age</div>
          )}
          <div className="lon-vital-row">
            <button className="lon-test-day-btn" onClick={e => { e.stopPropagation(); setTestMode(true); setTestStep(0); setInputValue(''); setInputNotes('') }}>🧪 Test Day</button>
            <div className="lon-age-inputs" onClick={e => e.stopPropagation()}>
