...
// Shared formatDesc — FIX #8: extracted from duplication
...
export default function WODCard({ workouts, session, onAuthRequired, onWorkoutsChanged, favorites, toggleFavorite, isAdmin, collections, onCollectionsChanged }) {
  ...
  // BUG FIX #3: Allow empty score (matches WorkoutCard behavior)
  ...
  // FIX #9: Use shared toSlug helper
  function copyLink() {
    if (!wod) return
    navigator.clipboard.writeText(`https://www.ronapump.com/workout/${toSlug(wod.name)}`)
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }
          ...
          <div className="det" onClick={e => e.stopPropagation()}>
            ...
            <div className="acts">
              ...
              <button className="ab" onClick={() => {
                window.location.href = '/workout/' + toSlug(wod.name) + '?remix=1'
              }}>🔀 Remix</button>
              ...
              {isAdmin && <button className="ab p" onClick={() => {
                window.location.href = '/workout/' + toSlug(wod.name)
              }}>Edit</button>}
              {isAdmin && <button className="ab del" onClick={deleteWorkout}>Delete</button>}
            </div>

            {/* Collections picker — FIX #7: now receives collections prop */}
